        ...
        me.$nextTick(() => {
          ...
          for (let i = 0; i < me.equipmentList.length; i++) {
            ...
            if (!row || (row && row.industryName == temp.industryName)) {
              ...
              let myIcon0 = new BMap.Icon("static/image/circle5.png", new BMap.Size(120, 120), {/*创建图标*/
              ...
              html0 += "</div></div>";
              let label0 = new BMap.Label(html0, { offset: new BMap.Size(-50, -50) });/*图片大小和文本大小的差值/2*/
              label0.setStyle({ width: "220px", height: "220px", cursor: "pointer", backgroundColor: "transparent", border: "none", display: "block" });
              let marker = new BMap.Marker(point, { icon: myIcon1 });
              marker.addEventListener("mouseover", function(e) {
                // if (me.currentMarker) {
                marker.setIcon(myIcon0);
                // if (marker.getLabel()) {
                label0.setStyle({ display: "block" });
                // }
                marker.setLabel(label0);
              });
              marker.addEventListener("mouseout", function(e) {
                marker.setIcon(myIcon1);
                label0.setStyle({ display: "none" });
              });
